...
tags: "notes to self, woodwork, maker, plywood"
---
...
I've been really into cooking lately, and I'm building a plywood box to hold all my spice jars. I initially made a very basic box (let's call it v1), but I made some design mis-steps in my impatience (namely just lasering it and gluing it together without thinking about sanding, etching, staining, varnishing, or testing how to attach a lid), 
...
Note: Not all tools are required for each step.
...
- Read the instructions (in my case, on [xHain's wiki]() but also Google Translating German instructions for stain, varnish, etc).
...
Since xHain's [laser cutter]() has a bed of 60cm by 30cm, I cut wood to size using the [table saw]() or [band saw]().

### Making Cut Marks

- First I measured with a tape measure, then marked (with a [carpenter's pencil]()) where to cut.
- Using a [set square](), I extended that line.
...
- (This is where I whould've used [tack cloth]() if I had any to remove any sanding residue).
...
- An [OpenSCAD design]() design I downloaded from [Thingiverse]() and then edited to match my design parameters.
- [Inkscape]() to arrange the pieces for cutting and [vectorize bitmap images]() for etching.
...
I watched many YouTube videos about staining plywood, but most of them covered oil-based stains. At xHain we only had a water-based stain to hand, and since I'm still new at staining, I didn't want to splash out for any new stuff. So water-based it was! I used [Clou]():
...
- Tape over edges to stop stain leaking underneath to other side. I want *both* sides stained, but leakage makes coloring inconsistent in my experience. I used painter's tape, but the water in the stain messed up it's adhesive in some spots, so I might use other tape next time.
- Put on latex gloves.
- Turn on ventilator.
...
- Stand vertically if you can to prevent stain leaking to the other side (I haven't actually tried this, but I will for my next session). I guess doing it so the grain is vertically-aligned would make sense, but at that point I don't think the stain is going anywhere.
